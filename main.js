...
countSelectorType = function(selector, count) {
...
countSelectors = function(rule, count) {
  if (rule.type === 'media' && rule.rules) {
    count.media++;
    _(rule.rules).each(function(rule) {
      return countSelectors(rule, count);
    });
  }
  if (_(rule).has('selectors') && rule.selectors.length > 0) {
    return _(rule.selectors).each(function(selector) {
      return countSelectorType(selector, count);
    });
  }
};
...
analyse = function(rules, count) {
  var selectors;
  count.rules = rules.length;
  return selectors = _(rules).each(function(rule) {
    return countSelectors(rule, count);
  });
};
...
init = function(data, lib) {
  var rules, _ref;
  lib.count.important = ((_ref = data.match(/\!important/gi)) != null ? _ref.length : void 0) || 0;
  ...
  _(rules).each(function(rule) {
    if (rule.selectors) {
      return _(rule.selectors).each(function(selector) {
        var nestLevel, _ref1;
        nestLevel = (_ref1 = selector.match(/\s|\>/gi)) != null ? _ref1.length : void 0;
        if (!lib.count.nesting[nestLevel] && nestLevel) {
          return lib.count.nesting[nestLevel] = 1;
        } else if (nestLevel) {
          return lib.count.nesting[nestLevel]++;
        }
      });
    }
  });
...
_(libraries).each(function(lib) {
  return ajaxRequest(lib.path, function(data, size) {
    lib.size = size;
    return init(data, lib);
  });
});
